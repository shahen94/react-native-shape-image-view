...
export class HexagonImage extends Component {
  ...
  animateViews(useAnimate) {
    ...
    let nextIndex = this.frameShowedCount === 2
      ? currImageIndex + 2
      : currImageIndex;
    ...
    Animated.parallel([
      ...
      if (showFrontImage) {
        // if frontal image is not visible let's change it silently
        nextState.currImageIndex = nextIndex;
      } else {
        // if background image is not visible let's change it silently
        nextState.nextImageIndex = prepareIndex;
      }

      this.setState(nextState, () => {
        // if (!this.showFrontImage) {
        //   this.state.nextImageIndex = prepareIndex;
        // } else {
        //   this.state.currImageIndex = nextIndex;
        // }
      });
    });
